...
type = sprintf('LOS_%s', name_dataset); % 'LOS_Positioning1' 'NLOS_Positioning1'
start_position = 100;
switch type 
    case sprintf('LOS_%s', name_dataset)
        boolean_los = 1;
    case sprintf('NLOS_%s', name_dataset)
        boolean_los = 0;
end
...
for pos = 1:numPos
    [status, msg, msgID] = mkdir(sprintf('%s/pos_%d', DB, pos));
end
...
switch FR
    ...
    case 2
        SCS = 60; % 120
        NSizeGrid = 273;
        fc = 4e9; %30e9;    % Carrier frequency (Hz)
end
...
'ElevationScanAngles',-90:.1:downtilt);
...
%% Start Simulation
for posIdx = start_position:numPos
    ...
    if numdetectedgNBs ~= 0
        ...
        if numdetectedgNBs ~= 0
            ...
            for gNBIdx = 1:numdetectedgNBs
                ...
                toc
                temp = doas(1,gNBIdx);
                % doas(1,gNBIdx) =  doas(2,gNBIdx);
                % doas(2,gNBIdx) = temp;
                doas(1,gNBIdx) = mod(doas(1,gNBIdx) + cellAngles(detectedgNBs(gNBIdx)), 360);
                doas(2,gNBIdx) = doas(2,gNBIdx) + downtilt;
                % figure(10000)
            ...
            sr = ofdmInfo.SampleRate; % sampling rate 
            for gNBIdx = 1: numdetectedgNBs
                [status, msg, msgID] = mkdir(sprintf('%s/pos_%d/cell_%d/ADCRM', DB, posIdx, detectedgNBs(gNBIdx)));
                ...
                ADCRM = ADCRM(:,1:7:252,:);
                save(sprintf('%s/pos_%d/cell_%d/ADCRM/abs_ADCRM.mat', DB, posIdx, detectedgNBs(gNBIdx)), 'ADCRM'); 
        ...
        %     % pause()
            ...
            % rho = rho(detectedgNBs);
            % rho = TOA./ofdmInfo.SampleRate*c;
            % estimatedPos_xyz = LS_TOA(rho, s, dimensions);

            % Compute CRB TDOA
            H = compute_H(u, s, dimensions, 'TDoA');
            sigma_TOA = compute_CRB_sigma_TOA(SNRLin, bandwidth);
            CRB_TDOA = compute_CRB_TOA(sigma_TOA, H);
            ...
            % Save CRB_TDOA
            save(sprintf('%s/pos_%d/CRB_TDOA.mat', DB, posIdx),'CRB_TDOA');
           
            % Save results of Estimate UE position with TDOA with each BS
            % coordinate reference
            for gNBIdx = 1: numdetectedgNBs

                UE_ML_realPos_xyz = - gNBPos_xyz{detectedgNBs(gNBIdx)}; % real UE pos is in [0,0,0] in UE coord system

                % [az,el,r] = cart2sph(UE_ML_realPos_xyz(1), UE_ML_realPos_xyz(2), UE_ML_realPos_xyz(3));
                % az = az*180/pi;
                % el = el*180/pi;

                % Save perfect measurements from BS
                AoA = angles_bs(:,detectedgNBs(gNBIdx));% - [cellAngles(detectedgNBs(gNBIdx)); - downtilt];
                ToA = true_dist(gNBIdx);
                save(sprintf('%s/pos_%d/cell_%d/measurements.mat', DB, posIdx, detectedgNBs(gNBIdx)), 'AoA', 'ToA'); 

                % Compute maximum likelihood position for each cell
                [x,y,z] = sph2cart(doas(1,gNBIdx)*pi/180,doas(2,gNBIdx)*pi/180,rho(gNBIdx)); % need global coordinate system
                est_AoA = doas(:,gNBIdx);
                est_ToA = rho(gNBIdx);
                save(sprintf('%s/pos_%d/cell_%d/est_measurements.mat', DB, posIdx, detectedgNBs(gNBIdx)), 'est_AoA', 'est_ToA'); 

                UE_ML_estimatedPos_xyz = [x, y, z];
                save(sprintf('%s/pos_%d/cell_%d/UE_ML_real_est_pos.mat', DB, posIdx, detectedgNBs(gNBIdx)), 'UE_ML_estimatedPos_xyz', 'UE_ML_realPos_xyz'); 

                % Compute CRB TOA + AOA
                H = compute_H(u, gNBPos_xyz{detectedgNBs(gNBIdx)}, dimensions, 'ToA');
                H = [H; compute_H(u, gNBPos_xyz{detectedgNBs(gNBIdx)}, dimensions, 'AoD')];
                sigma_TOA = compute_CRB_sigma_TOA(SNRLin, bandwidth);
                AoA = mod(AoA - [cellAngles(detectedgNBs(gNBIdx)); -downtilt], 360);
                sigma_AOA = compute_CRB_sigma_AOA(SNRLin, bandwidth, NRxAnts, 0.5*lambda, AoA*pi/180);
                CRB_TOA_AOA = compute_CRB_TOA_AOA(sigma_AOA, sigma_TOA, H);
                save(sprintf('%s/pos_%d/cell_%d/CRB_TOA_AOA.mat', DB, posIdx, detectedgNBs(gNBIdx)), 'CRB_TOA_AOA'); 
