...
final_metrics = {}
scores_per_batches_normal_train = []
scores_per_batches_anomaly_test = []
scores_per_batches_normal_test = []
NUM_EPOCHS = 30
...
for value in values:
    ...
    perform_training = 0
    type_model = 'Model3'          # 'DaGMM', 'Model1', 'Model2', 'Model3', 'BetaVAE_H', 'GANomaly'
    var_to_monitor = 'energy1'  # 'likelihood' 'energy'
    ...
    config = {
        ...
        'pretrained_model' : None ,  #  None '3_587' '10_586' 1173 '10_48'
        ...
        'log_step' : int(num_BATCHES/1),                    #  20 int(num_BATCHES/30)
        'model_save_step' : num_BATCHES, #  586
    }  

    solver = Solver(config)

    epochs_to_test = np.arange(NUM_EPOCHS, NUM_EPOCHS+1, 1)
    prove = 0
    log_scale = False
    logy = 0
    bins = 10
    ...
    for e in epochs_to_test:
        ...
        # plot_roc_auc_curve(fpr, tpr, round(roc_auc, 3),  dir = log_path )
        ...
        # acc, recall, precision, f_score = compute_thresholds(thrs = thr, gt = gt, scores = score)
        # index_best = f_score.index(max(f_score))
        ...
        try:
            if config['var_to_monitor'] == 'likelihood1':
                scores_per_batches_normal_train.append(solver.train_likelihood1.ravel())
                scores_per_batches_anomaly_test.append(solver.test_likelihood1[np.squeeze(solver.test_labels == 1)].ravel())
                scores_per_batches_normal_test.append(solver.test_likelihood1[np.squeeze(solver.test_labels == 0)].ravel())
            elif config['var_to_monitor'] == 'energy1':
                scores_per_batches_normal_train.append(solver.train_energy1.ravel())
                scores_per_batches_anomaly_test.append(solver.test_energy1[np.squeeze(solver.test_labels == 1)].ravel())
                scores_per_batches_normal_test.append(solver.test_energy1[np.squeeze(solver.test_labels == 0)].ravel())
            elif config['var_to_monitor'] == 'likelihood2':
                scores_per_batches_normal_train.append(solver.train_likelihood2.ravel())
                scores_per_batches_anomaly_test.append(solver.test_likelihood2[np.squeeze(solver.test_labels == 1)].ravel())
                scores_per_batches_normal_test.append(solver.test_likelihood2[np.squeeze(solver.test_labels == 0)].ravel())
            elif config['var_to_monitor'] == 'energy2':
                scores_per_batches_normal_train.append(solver.train_energy2.ravel())
                scores_per_batches_anomaly_test.append(solver.test_energy2[np.squeeze(solver.test_labels == 1)].ravel())
                scores_per_batches_normal_test.append(solver.test_energy2[np.squeeze(solver.test_labels == 0)].ravel())
        except:
            pass
    ...
    print(f'Value: {value}, AUC: {auc_}, Precision: {p}, Recall: {r}, F_score: {f}')

    del sys.modules['Classes.solver'] 
    reload(Classes)
    from Classes.solver import Solver
    del solver.model
    del solver.optimizer
    del solver

# scores_per_batches_normal_train = [np.exp(-el)-1 for el in scores_per_batches_normal_train]
# scores_per_batches_anomaly_test = [np.exp(-el)-1 for el in scores_per_batches_anomaly_test]
# scores_per_batches_normal_test = [np.exp(-el)-1 for el in scores_per_batches_normal_test]

try:
    # Print comparison between batches of normal data distribution
    plot_many_likelihood(scores_per_batches_normal_train, 
                        names=[ f'{value}' for value in values], 
                        log_path = DIR_EXPERIMENT, 
                        file_name = test_name + '_train_normal', 
                        prove = prove, 
                        log_scale = log_scale, 
                        logy = logy, 
                        bins=bins, 
                        ...
                        save_pdf = 1, 
                        plt_show = 0, 
                        var_to_monitor = config['var_to_monitor'])
except:
    pass

try:
    # Print comparison between batches of normal data distribution
    plot_many_likelihood(scores_per_batches_anomaly_test, 
                        names=[ f'{value}' for value in values], 
                        log_path = DIR_EXPERIMENT, 
                        file_name = test_name + '_test_anomaly', 
                        prove = prove, 
                        log_scale = log_scale, 
                        logy = logy, 
                        bins=bins, 
...
# try:
#     # Print comparison between batches of normal data distribution
#     plot_many_likelihood(scores_per_batches_normal_test, 
#                         names=[ f'{value}' for value in values], 
#                         log_path = DIR_EXPERIMENT, 
#                         file_name = test_name + '_test_normal', 
#                         prove = prove, 
#                         log_scale = log_scale, 
#                         bins=bins, 
#                         threshold = None, 
