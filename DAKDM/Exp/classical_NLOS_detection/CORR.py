...
from Classes.utils import mkdir, compute_avg_Pearson_coeff
...
for i, (input_data, labels) in enumerate(tqdm(train_loader)):
    ...
    if delay_peak_end < sample.shape[0]:
        ref_samples.append(sample[delay_peak_start:delay_peak_end, angle_peak])
...
for i, (input_data, labels) in enumerate(tqdm(val_loader)):
    ...
    if delay_peak_end < sample.shape[0]:
        score.append(compute_avg_Pearson_coeff(sample[delay_peak_start:delay_peak_end, angle_peak], ref_samples))
...
np.save(DIR_EXPERIMENT + '/final_metrics_CORR.npy', final_metrics, allow_pickle = True)
