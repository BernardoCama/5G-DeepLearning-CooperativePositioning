...
for i, (input_data, labels) in enumerate(tqdm(val_loader)):
    ...
    if i>=num_samples_training:
        ...
        angle_peak = delay_peak[1][0]
        delay_peak = delay_peak[0][0]
        features = np.array(extract_features_CIR(sample[:, angle_peak], sample[delay_peak, :])).reshape([1,-1])[0]
        # plot_reconstructed_samples (input_=sample, sample_number=0, label=0, log_path='', save_pdf = 0, save_eps = 0, file_name = '')
        score.append(np.prod([pdfs_LOS[index].pdf(feat) for index,(feat) in enumerate(features)])/np.prod([pdfs_NLOS[index].pdf(feat) for index,(feat) in enumerate(features)]))
